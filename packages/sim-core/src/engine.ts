...
function buildTeam(teamId: 'A' | 'B', rng: XorShift32): Player[] {
  const formation = [
  ...
  return formation.map((p, i) => {
    ...
    for (const k in stats) stats[k as keyof PlayerStats] = v2.clamp((stats[k as keyof PlayerStats] as number) + rng.range(-8, 8), 10, 99);
...
export class FootballEngine {
  ...
  tick(dtSec: number): void {
    ...
    this.allPlayers.forEach(p =>
      p.update(dtSec, this.ball, this.allPlayers.filter(t => t.team === p.team && t !== p), this.allPlayers.filter(t => t.team !== p.team))
    );
  ...
  private _decide(player: Player): void {
  ...
  private _decideWithoutBall(player: Player, teammates: Player[], opponents: Player[], goalPos: Vec2): void {
  ...
  private _shoot(player: Player, goalPos: Vec2): void {
    ...
    const speed =
      420 + (player.stats.shooting / 100) * 160 - (dist / 300) * 60;
  ...
  private _pass(player: Player, receiver: Player): void {
    const ball = this.ball;
  ...
  private _findInterceptor(from: Vec2, to: Vec2, opponents: Player[]): Player | null {
  ...
  private _distToSegment(p: Vec2, a: Vec2, b: Vec2): number {
    const ab = v2.sub(b, a);
    const ap = v2.sub(p, a);
  ...
  private _calcPressure(player: Player, opponents: Player[]): number {
    let p = 0;
  ...
  private _checkBallOut(): void {
    ...
    const throwInPlayer: Player | undefined = this.allPlayers.find(p => p.team === throwInTeam && v2.dist(p.pos, this.ball.pos) < 150);
    if (throwInPlayer) {
